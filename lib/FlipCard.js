...
export default class FlipCard extends Component {
  ...
  constructor (props) {
    ...
    const isFlipped = (props.alignHeight || props.alignWidth) ? !props.flip : props.flip;
  ...
  render () {
    ...
    let render_side = false

    const direction = this.props.invert ? '-180deg' : '180deg';
    ...
    if (this.state.isFlipped) {
      render_side = (
        <Back
          ...
          onLayout={(event) => {
            ...
            const _update = Object.assign(this.state.back, {width: width, height: height})
      ...
      render_side = (
        <Face
          ...
          onLayout={(event) => {
            const {x, y, width, height} = event.nativeEvent.layout;
            const _update = Object.assign(this.state.face, {width: width, height: height})
